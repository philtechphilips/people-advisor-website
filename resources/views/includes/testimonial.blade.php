<section class="md:px-24 px-5 mb-10">
    <h1 class="text-center font-bold text-5xl">See why our clients trust us</h1>
    <div class="swiper">
        <div class="flex swiper-wrapper justify-between mt-16 md:gap-6">

            <div
                class="md:w-[373px] 2xl:w-[400px] md:h-[380px] border border-[#54586180] rounded-[32px] py-10 px-6 swiper-slide">
                <h4 class="w-fit text-xl font-bold"
                    style="background: linear-gradient(142.53deg, #075097 41.81%, #F16823 74.05%); -webkit-background-clip: text;
    -webkit-text-fill-color: transparent;">
                    “Transformational HR Solutions”</h4>
                <p class="text-[#7C7C7C] mt-2 mb-4">Partnering with People Advisor has been a game-changer for our
                    company.
                    Their HR consulting services have streamlined our payroll management and enhanced our employee
                    engagement strategies. Their team is knowledgeable, responsive, and truly understands our business
                    needs
                </p>

                <h6 class="text-[#191B1F] font-bold">Tunde Oladimeji</h6>
                <p class="text-[#7C7C7C]">CEO of A1 Pinnacle Apartment</p>
            </div>


            <div
                class="md:w-[373px] 2xl:w-[400px] md:h-[380px] border border-[#54586180] rounded-[32px] py-10 px-6 swiper-slide">
                <h4 class="w-fit text-xl font-bold"
                    style="background: linear-gradient(142.53deg, #075097 41.81%, #F16823 74.05%); -webkit-background-clip: text;
    -webkit-text-fill-color: transparent;">
                    “Expert Social Media Advisory”</h4>
                <p class="text-[#7C7C7C] mt-2 mb-4">
                    We saw a significant improvement in our social media presence after working with People Advisor.
                    Their
                    tailored strategy and content creation have not only increased our followers but also boosted our
                    engagement rates. They have a deep understanding of the digital landscape
                </p>

                <h6 class="text-[#191B1F] font-bold">Kolawole Odunfa</h6>
                <p class="text-[#7C7C7C]">Lead Consultant Magnum Rehabilitation Ltd</p>
            </div>



            <div
                class="md:w-[373px] 2xl:w-[400px] md:h-[380px] border border-[#54586180] rounded-[32px] py-10 px-6 swiper-slide">
                <h4 class="w-fit text-xl font-bold"
                    style="background: linear-gradient(142.53deg, #075097 41.81%, #F16823 74.05%); -webkit-background-clip: text;
    -webkit-text-fill-color: transparent;">
                    “Exceptional Service and Support”</h4>
                <p class="text-[#7C7C7C] mt-2 mb-4">The team at People Advisor is incredibly professional and always
                    goes
                    above and beyond to meet our needs. Their comprehensive consulting services have made a significant
                    impact on our business, and we couldn't be more pleased with the results</p>

                <h6 class="text-[#191B1F] font-bold">Adebowala Idris</h6>
                <p class="text-[#7C7C7C]">Executive director Xttreme</p>
            </div>

            <div
                class="md:w-[373px] 2xl:w-[400px] md:h-[380px] border border-[#54586180] rounded-[32px] py-10 px-6 swiper-slide">
                <h4 class="w-fit text-xl font-bold"
                    style="background: linear-gradient(142.53deg, #075097 41.81%, #F16823 74.05%); -webkit-background-clip: text;
    -webkit-text-fill-color: transparent;">
                    “Transformational HR Solutions”</h4>
                <p class="text-[#7C7C7C] mt-2 mb-4">Partnering with People Advisor has been a game-changer for our
                    company.
                    Their HR consulting services have streamlined our payroll management and enhanced our employee
                    engagement strategies. Their team is knowledgeable, responsive, and truly understands our business
                    needs
                </p>

                <h6 class="text-[#191B1F] font-bold">Tunde Oladimeji</h6>
                <p class="text-[#7C7C7C]">CEO of A1 Pinnacle Apartment</p>
            </div>
        </div>

        <div class="flex w-full items-center justify-between gap-6 mt-6">
            <!-- Swiper Scrollbar on the Left -->
            <div class="swiper-scrollbar  justify-start w-[60%]"></div>

            <!-- Buttons on the Right -->
            <div class="w-[40%] flex justify-end gap-4">
                <div
                    class="bg-[#F1F2F3] swiper-button-prev-1 border border-[#C9CCCF] rounded-full h-14 w-14 flex items-center justify-center">
                    <i class="ri-arrow-left-line text-xl text-[#1D1F21]"></i>
                </div>

                <div
                    class="bg-[#075097] swiper-button-next-1 border border-[#C9CCCF] rounded-full h-14 w-14 flex items-center justify-center">
                    <i class="ri-arrow-right-line text-xl text-white"></i>
                </div>
            </div>
        </div>

    </div>
</section>
